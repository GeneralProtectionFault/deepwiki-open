...
def check_ollama_model_exists(model_name: str, ollama_host: str = None) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Allow match with or without tag
...
class OllamaDocumentProcessor(DataComponent):
    ...
    """
    def __init__(self, embedder: adal.Embedder, progress_callback: Optional[Callable[[int, int], None]] = None) -> None:  
        super().__init__()  
    ...
    def __reduce__(self):
        ...
        # model_client chain (adalflow's OllamaClient + its lazy-import proxy)
        # cannot be safely pickled. By the time save_state() runs, __call__
        # has already computed and attached .vector to every Document, so the
        # embedder is not needed again on reload. Reconstruct with embedder=None.
